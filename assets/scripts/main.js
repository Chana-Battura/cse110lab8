  ...
  if ("serviceWorker" in navigator){
    ...
    window.addEventListener("load", (event) => {     
      ...
      navigator.serviceWorker.register("/sw.js").then(  
        ...
        (registration) => {
          console.log("Sucessfully registered service worker.  Here is registration status", registration);
        },
        (error) => {
        // B5. TODO - In the event that the service worker registration fails, console
        //            log that it has failed.
          console.error(`Failed registering service worker with following error: ${error}`);
        },
      );
    });
  }
...
async function getRecipes() {
  ...
  if (loaded_recipes.length == RECIPE_URLS.length){
    saveRecipesToStorage(loaded_recipes);
    resolve(loaded_recipes);
  }
